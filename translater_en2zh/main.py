...
from translater_en2zh import catalog
from translater_en2zh.runner import Runner
from translater_en2zh.config import load_model, load_config
...
def main():
  ...
  parser.add_argument("--ae_model", default="",
                      help="Custom AutoEncoder model configuration file.")
  ...
  ae_model = None
  if args.ae_model and config.get("ae_model_dir") != None and config.get("ae_data") != None:
    ae_model = load_model(
                config["ae_model_dir"],
                model_file=args.ae_model,
                model_name=None,
