...
from opennmt.utils.data import inference_pipeline
...
from translater_en2zh import text_inputter
from translater_en2zh.decoders.decoder import get_sampling_probability
...
class SequenceToSequence(Model):
  ...
  def __init__(self,
    ...
    if not isinstance(target_inputter, text_inputter.WordEmbedder):
...
class SequenceToSequenceInputter(inputters.ExampleInputter):
  ...
  def make_inference_features(self, line):
    sp_line = tf.string_split([line], "\t").values
    tokens_trans = self.features_inputter.tokenizer.tokenize(sp_line[0])
    tokens_ae = self.labels_inputter.tokenizer.tokenize(sp_line[1])
    tokens_ori = self.labels_inputter.tokenizer.tokenize(sp_line[2])
    features = dict()
    features["ids"] = self.features_inputter.vocabulary.lookup(tokens_trans)
    features["length"] = tf.shape(tokens_trans)[0]
    features["tokens"] = tokens_trans
    features["ids_ae"] = self.labels_inputter.vocabulary.lookup(tokens_ae)
    features["length_ae"] = tf.shape(tokens_ae)[0]
    features["tokens_ae"] = tokens_ae
    features["ids_ori"] = self.labels_inputter.vocabulary.lookup(tokens_ori)
    features["length_ori"] = tf.shape(tokens_ori)[0]
    features["tokens_ori"] = tokens_ori
    return features
...
if __name__ == "__main__":
    ...
    inp = SequenceToSequenceInputter(
                text_inputter.WordEmbedder(vocabulary_file_key="source_words_vocabulary", embedding_size=512),
                text_inputter.WordEmbedder(vocabulary_file_key="target_words_vocabulary", embedding_size=512),
                share_parameters = EmbeddingsSharingLevel.share_input_embeddings(EmbeddingsSharingLevel.NONE),
                alignment_file_key = "train_alignments"
            )
    inp.initialize(metadata)
    dataset = inp.make_inference_dataset(
                "../data/test/test.triple",
                batch_size=16
            )
    ...
    print (features.keys())
    print ("=============== trans ================")
    print ((b" ".join(features["tokens"][0])).decode("utf-8"))
    print (features["ids"][0])
    print (features["length"][0])
    print ("=============== ae ================")
    print ((b" ".join(features["tokens_ae"][0])).decode("utf-8"))
    print (features["ids_ae"][0])
    print (features["length_ae"][0])
    print ("=============== ori ================")
    print ((b" ".join(features["tokens_ori"][0])).decode("utf-8"))
    print (features["ids_ori"][0])
    print (features["length_ori"][0])
