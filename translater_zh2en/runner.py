...
from translater_zh2en import estimator as estimator_util
...
class Runner(object):
  ...
  def __init__(self,
    ...
    if (self._ae_model != None):
        self._ae_model.initialize(self._config["ae_data"])
    tf.logging.info(
  ...
  def _make_estimator(self):
    ...
    if (self._ae_model != None):
      ...
      run_config = tf.estimator.RunConfig(
          #model_dir=self._config["ae_model_dir"],
    ...
    params.setdefault("batch_size", self._config["infer"]["batch_size"])
    params.setdefault("ae_init_checkpoint", self._config["ae_model_dir"])
    params.setdefault("trans_init_checkpoint", self._config["model_dir"])
