...
class GreedySearch(DecodingStrategy):
  ...
  def step(self,
    ...
    return sample_ids, cum_log_probs, finished, state_trans, state_ae, extra_vars
...
class BeamSearch(DecodingStrategy):
  ...
  def step(self,
    ...
    if accumulated_attention is not None:
      ...
      extra_vars.append(accumulated_attention)
    state_trans = compat.nest.map_structure(lambda s: _gather_state(s, beam_indices), state_trans)
    state_ae    = compat.nest.map_structure(lambda s: _gather_state(s, beam_indices), state_ae   )
    return word_ids, cum_log_probs, finished, state_trans, state_ae, tuple(extra_vars)
...
def dynamic_decode(symbols_to_logits_fn_trans,
  ...
  def _cond(step, finished, state_trans, state_ae, inputs, outputs, attention, cum_log_probs, extra_vars):  # pylint: disable=unused-argument
  ...
  _, _, state_trans, state_ae, _, outputs, attention, log_probs, extra_vars = tf.while_loop(
      ...
      shape_invariants=(
          step.shape,
          finished.shape,
          compat.nest.map_structure(_get_shape_invariants, initial_state_trans),
          compat.nest.map_structure(_get_shape_invariants, initial_state_ae),
          start_ids.shape,
          tf.TensorShape(None),
          tf.TensorShape(None),
